...
| **The write path** | **moved to the console in #44.** `POST /api/proposal` and `/api/proposal/op` go through `console/lib/door.ts` — the closed 17-op vocabulary, capability, the content address and the `au:Verdict`. The plugin's two copies answer **410 Gone** naming them, because two doors are two implementations of the address and they had already diverged on `bound_value`. `POST /proposal/verdict-preview` stays in `src/proposal.rs` and writes nothing; it returns the address the console would give the proposal. The GATE verdict is still the build's — RFC-002 §7.1 has the table |
