...
# worker — same pattern as plugin-mycelium / plugin-tbzl). spec-server has NO private
# git-deps (v1 inlines the facade, no fastverk-plugin-crates), so no BuildKit secret.
#
#   docker buildx build --platform linux/amd64 -t <ecr>/spec-server:<tag> --push .
#
...
WORKDIR /src
COPY . .
# protoc backs the plugin's build.rs (spec.v1 prost-build); build-essential backs ring.
RUN cargo build --release --bin spec-server
