...
\begin{frame}{The corpus is a near-star around conservation}
  \begin{columns}[c]
    \begin{column}{0.66\textwidth}
      \centering
      % TikZ, not \includegraphics: rules_graphviz's svg_pdf/dot_pdf cannot
      % build for any consumer (see //graph:BUILD.bazel). \resizebox is exact
      % here rather than an approximation of keepaspectratio -- the layout is
      % 11.24x10.49in against a 0.66\textwidth column, so height is the
      % binding constraint, as it was for the PDF.
      \resizebox{!}{0.78\textheight}{\input{../graph/crystal_tikz.tex}}
    \end{column}
...
\begin{frame}{The symmetry collapse: 16 specs are one orbit}
  \begin{columns}[c]
    \begin{column}{0.47\textwidth}
      \centering
      % 11.66x8.63in in a 0.47\textwidth column: height binds, so the single
      % \resizebox dimension reproduces width+height+keepaspectratio exactly.
      \resizebox{!}{0.60\textheight}{\input{../graph/collapse_before_tikz.tex}}\\[2pt]
    ...
    \begin{column}{0.47\textwidth}
      \centering
      % 6.59x7.98in -- taller than wide, so height binds here too.
      \resizebox{!}{0.60\textheight}{\input{../graph/collapse_after_tikz.tex}}\\[2pt]
